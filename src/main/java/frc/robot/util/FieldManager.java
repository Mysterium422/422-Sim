...
public class FieldManager extends SubsystemBase {
    ...
    public void periodic() {
        if (driveSimulation.getSimulatedDriveTrainPose().getTranslation().getDistance(FieldConstants.CoralStation.rightCenterFace.getTranslation()) < 0.5) {
            if (!atRightStation) {
                SimulatedArena.getInstance()
                    .addGamePieceProjectile(MapleUtil.generateCoralAtPoint(
                            ...
                            new Translation3d(2.3, new Rotation3d(Degrees.zero(), Degrees.of(35), Degrees.of(45)))
                        )
                    );
            }
            atRightStation = true;
        ...
        if (driveSimulation.getSimulatedDriveTrainPose().getTranslation().getDistance(FieldConstants.CoralStation.leftCenterFace.getTranslation()) < 0.5) {
            if (!atLeftStation) {
                SimulatedArena.getInstance()
                    .addGamePieceProjectile(MapleUtil.generateCoralAtPoint(
                            new Pose3d(
